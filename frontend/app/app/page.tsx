...
import { Panel, Field, RunButton, ErrorNote, ResultCard, Pre, SectionLabel } from "@/components/Panel";
import { inputStyle } from "@/components/Panel";
  ...
  return (
    <main className="command-center">
      <div className="command-center__top">
        <a href="/">← CareerOS</a>
        <span style={{ opacity: 0.55, fontSize: 13, letterSpacing: "0.08em" }}>
          COMMAND CENTER
        </span>
      </div>
...
function ProfileModule() {
  ...
  async function run() {
    ...
    try {
      const r = await api.profiles.upsert({
        email, fullName,
...
/* 2/3. Job Discovery — live search */
function JobDiscoveryModule() {
  const [q, setQ] = useState("python developer");
  ...
  async function run() {
    setLoading(true); setError(null);
    try {
      const r = await api.jobs.search(q, { location: loc, useAdzuna: false });
      setJobs(r.jobs || []);
    } catch (e: any) { setError(e.message); }
    finally { setLoading(false); }
  }
  ...
  return (
    <Panel index="02" eyebrow="Job Discovery" title="Live job search">
      <Field label="Query"><input style={inputStyle} value={q} onChange={(e) => setQ(e.target.value)} /></Field>
      <Field label="Location (optional)"><input style={inputStyle} value={loc} onChange={(e) => setLoc(e.target.value)} /></Field>
      <RunButton onClick={run} loading={loading} disabled={!q}>Search live jobs</RunButton>
      <ErrorNote error={error} />
      {jobs.length > 0 && (
        <ResultCard>
          <p style={{ opacity: 0.6, fontSize: 13 }}>{jobs.length} live postings</p>
          <div className="chip-row">
            {jobs.slice(0, 12).map((j, i) => (
              <a key={i} className="chip" href={j.url} target="_blank" rel="noreferrer">
                {j.title?.slice(0, 40)} · {j.company?.slice(0, 18)}
              </a>
            ))}
          </div>
        </ResultCard>
...
function JobAlertsModule() {
  ...
  async function run(scan = false) {
    ...
    try {
      ...
      const n = await api.notifications.list(email);
      setItems(n);
    } catch (e: any) { setError(e.message); }
    finally { setLoading(false); }
  }
...
function AtsModule() {
  ...
  async function run() {
    setLoading(true); setError(null);
    try {
      const tr = safeJson(tailored, "tailoredResume");
      const jj = safeJson(jdJson, "jobJson");
      setResult(await api.resume.atsAnalyze(tr, jj));
    } catch (e: any) { setError(e.message); }
    finally { setLoading(false); }
  }
...
function MissingSkillsModule() {
  ...
  async function run() {
    setLoading(true); setError(null);
    try {
      setResult(await api.resume.missingSkills(safeJson(tailored, "tailoredResume"), safeJson(jdJson, "jobJson")));
    } catch (e: any) { setError(e.message); }
    finally { setLoading(false); }
  }
...
function ApplicationTrackerModule() {
  const [email, setEmail] = useState("");
  const [company, setCompany] = useState("");
  ...
  async function add() {
    setLoading(true); setError(null);
    try {
      await api.applications.add({ candidateEmail: email, company, role, status: "Applied" });
      await load();
    } catch (e: any) { setError(e.message); }
    finally { setLoading(false); }
  }
...
function safeJson(s: string, field: string) {
  if (!s.trim()) throw new Error(`${field} is required`);
  try { return JSON.parse(s); }
  catch {
    // allow plain text for resume-ish fields
    return { raw: s };
  }
}
